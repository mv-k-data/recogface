...
def about():
    ...
    history = [{"uuid":i[0], "image_name":i[1], "created_at": i[2], "search_engines": i[3], "count": i[4]} for i in history]
...
def upload_file():
    response = {
        "error": "",
        "content": "",
        "original_image": "static/images/img1.png"
    }
    ...
    if file and allowed_file(file.filename):
        ...
        try:
            search_engine = SearchEngineFactory.get_search_engine(engine, request_id, original_image_path)
            result_list = search_engine.search_images()
        except Exception as e:
            response["error"] = f"Error: {str(e)}"
            return jsonify(response), 200
        search_results = [(i["uuid"], i["search_engine"], i["image_name"], i["image_url"], i["image_text"], i["full_image_name"]) for i in result_list]
        db.save_search_result(search_results=search_results)

        response["content"] = render_template("includes/_list_items_cards.html", items=result_list)
